		...
		if ( array_key_exists( $posttype, (array) $post_types ) ) {
			...
			if ( $r->have_posts() ) : ?>
				...
				<?php while ( $r->have_posts() ) : $r->the_post(); ?>
					<div class="partner"><a href="<?php echo $key_1_value = get_post_meta( get_the_ID(), 'partnerlink', true );?>" target="_blank"><img src="<?php echo $partnerimage = wp_get_attachment_url( get_post_thumbnail_id( $post->ID ) );?>" target="_blank" alt="<?php get_the_title() ? the_title() : the_ID(); ?>"></a>
					</div>
				<?php endwhile; ?>
				</div>
